class Dashboard extends React.Component {
  ...
  reload(e) {
    e.preventDefault();
    this.getPrices(this.state.hotel_id, this.state.competitor_id);
    return false;
  }
  ...
  renderTable () {
    ...
    dateRows = _.map(this.state.dates, (date, index) => <th key={index}>{date}</th>)
    roomRows = _.map(this.state.prices, (prices, room) => <Room key={room} room={room} prices={prices} dates={this.state.dates} payLater={this.state.payLater} />)
    return (
      <table className="table table-hover table-striped table-bordered margin-top-small">
        <thead>
          <tr>
            <th className="vertical-middle">Loại phòng</th>
            {dateRows}
          </tr>
        </thead>
        ...
        <form className="add-form" onSubmit={this.onCrawlPrices.bind(this)}>
          ...
          <div className="form-group row">
            ...
            <div className="col-sm-4 text-center">
              ...
              <button className="btn btn-default btn-small margin-left-small" onClick={this.reload.bind(this)}>Tải lại</button>
            </div>
          </div>
        </form>
